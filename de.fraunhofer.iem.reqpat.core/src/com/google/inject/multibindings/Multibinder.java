 ...
 * 
 * <p>
 * The set is unmodifiable. Elements can only be added to the set by configuring
 * the multibinder. Elements can never be removed from the set.
 * 
 * <p>
...
public abstract class Multibinder<T> {
	 ...
	 * of the set's elements. Each binding is given an annotation that
	 ...
	 * is bound. Since the list of bindings is retrieved from the injector
	 * itself (and not the multibinder), each multibinder has access to all
	...
	static final class RealMultibinder<T> extends Multibinder<T> implements
			Module, ProviderWithExtensionVisitor<Set<T>>, HasDependencies,
		...
		/*
		 * a binding for each element in the set. null until initialization,
		 * non-null afterwards
		 */
		...
		public <B, V> V acceptExtensionVisitor(
			...
			if (visitor instanceof MultibindingsTargetVisitor) {
				return ((MultibindingsTargetVisitor<Set<T>, V>) visitor)
						.visit(this);
			} else {
		...
		public LinkedBindingBuilder<T> addBinding() {
			checkConfiguration(!isInitialized(),
					"Multibinder was already initialized");
		...
		public Set<T> get() {
			...
			for (Binding<T> binding : bindings) {
				...
				checkConfiguration(newValue != null,
						"Set injection failed due to null element");
				checkConfiguration(
						result.add(newValue) || permitDuplicates,
		...
		public Set<Dependency<?>> getDependencies() {
			if (!isInitialized()) {
				return ImmutableSet.<Dependency<?>> of(Dependency.get(Key
						.get(Injector.class)));
			} else {
				return dependencies;
			}
		 ...
		 * each element in this set. At this time the set's size is known, but
		 * its contents are only evaluated when get() is invoked.
		 */
		@Toolable
		@Inject
		...
		private boolean keyMatches(Key<?> key) {
			return key.getTypeLiteral().equals(elementType)
					...
					&& ((Element) key.getAnnotation()).setName()
							.equals(setName);
		}
		...
		private String nameOf(Key<?> key) {
			...
			Class<? extends Annotation> annotationType = setKey
					.getAnnotationType();
		...
		public String toString() {
			return new StringBuilder().append(setName)
					...
					.append("Multibinder<").append(elementType).append(">")
					.toString();
		}
	}
	...
	static void checkConfiguration(boolean condition, String format,
		...
		throw new ConfigurationException(ImmutableSet.of(new Message(Errors
				.format(format, args))));
	}
	...
	static <T> T checkNotNull(T reference, String name) {
		...
		throw new ConfigurationException(ImmutableSet.of(new Message(
				ImmutableList.of(), npe.toString(), npe)));
	}
	 ...
	 */
	public static <T> Multibinder<T> newSetBinder(Binder binder,
			TypeLiteral<T> type, Annotation annotation) {
		binder = binder.skipSources(RealMultibinder.class, Multibinder.class);
		RealMultibinder<T> result = new RealMultibinder<T>(binder, type,
	 ...
	 */
	public static <T> Multibinder<T> newSetBinder(Binder binder,
			TypeLiteral<T> type, Class<? extends Annotation> annotationType) {
		binder = binder.skipSources(RealMultibinder.class, Multibinder.class);
		RealMultibinder<T> result = new RealMultibinder<T>(binder, type,
